...
async def get_game_comparison_route(game_id):
    ...
    comparison_data = await comparison_service.get_game_comparison(
        game_id, lookback_games=lookback_games
    )

    if "error" in comparison_data:
        status_code = 500
        if "Could not fetch" in comparison_data.get(
            "error", ""
        ) or "Missing team ID" in comparison_data.get("error", ""):
            status_code = 404
...
@comparison_bp.route("/full-game/<int:game_id>", methods=["GET"])
async def get_full_game_comparison_route(game_id):
    """Returns a full-game (9-inning) comparison between the two teams."""
    lookback_games = request.args.get("games", default=10, type=int)

    comparison_data = await comparison_service.get_full_game_comparison(
        game_id,
