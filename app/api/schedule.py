...
async def today_schedule_route():
    """Gets the schedule for today, including pitcher info."""
    return jsonify(await schedule_service.get_today_schedule())


@schedule_bp.route("/recent_results", methods=["GET"])
async def recent_results_route():
    """Gets completed results for today and yesterday."""
    return jsonify(await schedule_service.get_recent_results())


@schedule_bp.route("/schedule/<int:team_id>", methods=["GET"])
async def get_schedule_route(team_id):
    """Gets historical schedule for a team, including pitcher info."""
    return jsonify(await schedule_service.get_schedule_for_team(team_id))


@schedule_bp.route("/next_schedule/<int:team_id>", methods=["GET"])
async def get_next_schedule_route(team_id):
    """Gets the upcoming schedule (today or tomorrow) for a team."""
    return jsonify(await schedule_service.get_next_game_schedule_for_team(team_id))
...
async def get_stats_batch_route(team_id: int, num_games: int):
    ...
    stats_summary = await game_service.get_team_stats_summary(
        team_id, num_games, include_details=True
    )
...
async def get_full_stats_summary(team_id: int, num_games: int):
    stats_summary = await game_service.get_team_stats_full_gamesummary(
        team_id, num_games, include_details=True
    )
    if isinstance(stats_summary, dict) and stats_summary.get("error"):
        return jsonify(stats_summary), 500
